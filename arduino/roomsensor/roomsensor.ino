...
// See KiCad schematic
static const byte PIN_DEBUG_LED = A3;
...
static const uint8_t MOTION_OFF = 255;
...
volatile bool readCcs881 = true;
volatile bool readBme280 = true;
...
void setup()
{
  ...
  Konnekting.init(Serial, PIN_SW1, PIN_DEBUG_LED, MANUFACTURER_ID, DEVICE_ID, REVISION);
  ...
  while(ccs811.setDriveMode(config.ccs811Mode) != CCS811Core::SENSOR_SUCCESS)
  {
    blinkLed(ERROR_CCS811);
    delay(1000);
  }
  while(ccs811.enableInterrupts() != CCS811Core::SENSOR_SUCCESS)
  {
    blinkLed(ERROR_CCS811);
    delay(1000);
  }
...
void loop()
{
  ...
  if(readMax44009)
  {
    ...
    if(max44009Lux >= MAX44009_LUX_MIN && max44009Lux <= MAX44009_LUX_MAX)
    {
      ...
      checkMaxLimit(max44009Lux, config.max44009LuxMaxLimit, config.max44009LuxTriggeredValue, COMOBJ_luxMaxLimitReached);
