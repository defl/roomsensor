...
static const byte PIN_MAX44009_INTERRUPT = 3;  // PD3,PCINT19 input
...
struct Config {
  ...
  float    bme280PressMinLimit;
  uint8_t  bme280PressMinTriggeredValue;
  float    bme280PressMaxLimit;
  uint8_t  bme280PressMaxTriggeredValue;
  
  float    max44009LuxDiffReportingThresholdPct;
  float    max44009LuxMaxLimit;
  uint8_t  max44009LuxTriggeredValue;
} config;


volatile bool readCcs881 = false;
volatile bool readBme280 = false;
...
// Sets previous value.
// returns bool true if written
template<typename T> bool writeIfDiff(T value, T* previousValue, T reportingThreshold, uint8_t commObject)
{
...
// Interrupt handler for A0-A5 (Only handle: A1=PIN_CCS811_INTERRUPT)
ISR (PCINT1_vect)
{
  if(digitalRead(PIN_CCS811_INTERRUPT) == HIGH)
    readCcs881 = true;
}  


// Interrupt handler for D0-D7 (Only handle: 3=PIN_MAX44009_INTERRUPT, 5=PIN_AS312_1_MOTION, 6=PIN_AS312_2_MOTION)
ISR (PCINT2_vect)
{
  if(digitalRead(PIN_MAX44009_INTERRUPT) == HIGH)
    readMax44009 = true;
...
void setup()
{
  ...
  pinMode(PIN_CCS811_INTERRUPT, INPUT);
  pinMode(PIN_CCS811_WAKE, OUTPUT);
  pinMode(PIN_SW1, INPUT);
  pinMode(PIN_MAX44009_INTERRUPT, INPUT);
  ...
  digitalWrite(PIN_SW1, HIGH); // KonnektingDevice.cpp ties this to RISING external input, pullup is is commented out in their code
  digitalWrite(PIN_MAX44009_INTERRUPT, HIGH); // MAX44009 will drive this low if data is ready
  ...
  if (!Konnekting.isFactorySetting())
  {
    ...
    config.bme280PressMinLimit = static_cast<float>(Konnekting.getUINT16Param(PARAM_bme280PressMinLimit));
    config.bme280PressMinTriggeredValue = Konnekting.getUINT8Param(PARAM_bme280PressMinTriggeredValue);
    config.bme280PressMaxLimit = static_cast<float>(Konnekting.getUINT16Param(PARAM_bme280PressMaxLimit));
    config.bme280PressMaxTriggeredValue = Konnekting.getUINT8Param(PARAM_bme280PressMaxTriggeredValue);
    ...
    config.max44009LuxMaxLimit = static_cast<float>(Konnekting.getUINT32Param(PARAM_max44009LuxMaxLimit));
    config.max44009LuxTriggeredValue = Konnekting.getUINT8Param(PARAM_max44009LuxTriggeredValue);
  }
  ...
  PCMSK2 |= bit(PCINT16);  // RXD, PD0
  PCMSK2 |= bit(PCINT19);  // PIN_MAX44009_INTERRUPT, PD3
  PCMSK2 |= bit(PCINT21);  // PIN_AS312_1_MOTION, PD5
  PCMSK2 |= bit(PCINT22);  // PIN_AS312_2_MOTION, PD6
  ...
  // BME280 read interrupt timer
  Timer1.initialize(config.bme280PollingIntervalMs * 1000);
  Timer1.attachInterrupt(interrupt_bme280_timer);
...
void loop()
{
  ...
  uint16_t ccs881eCo2;
  uint16_t ccs881TVoc;
  static uint16_t ccs881eCo2Previous;
  static uint16_t ccs881TVocPrevious;
  ...
  if(readCcs881)
  {
    ...
    if(ccs881eCo2 >= CCS811_ECO2_MIN && ccs881eCo2 <= CCS811_ECO2_MAX && 
       ccs881TVoc >= CCS811_TVOC_MIN && ccs881TVoc <= CCS811_TVOC_MAX)
    {
      writeIfDiff(ccs881eCo2, &ccs881eCo2Previous, config.ccs811Eco2DiffReportingThreshold, COMOBJ_eco2Value);
      writeIfDiff(ccs881TVoc, &ccs881TVocPrevious, config.ccs811TvocDiffReportingThreshold, COMOBJ_tvocValue);
  ...
  float bme280Rh;
  float bme280Press;
  float bme280TempPrevious = 0;
  float bme280RhPrevious = 0;
  float bme280PressPrevious = 0;

  if(readBme280)
  {
    bme280.read(bme280Press, bme280Temp, bme280Rh, BME280::TempUnit_Celcius, BME280::PresUnit_hPa);

    if(bme280Temp >= BME280_TEMP_MIN && bme280Temp <= BME280_TEMP_MAX &&
       bme280Rh >= BME280_HUM_MIN && bme280Rh <= BME280_HUM_MAX &&
       bme280Press >= BME280_PRESS_MIN && bme280Press <= BME280_PRESS_MAX)
    {
      writeIfDiff(bme280Temp, &bme280TempPrevious, config.bme280TempDiffReportingThreshold, COMOBJ_tempValue);
      writeIfDiff(bme280Rh, &bme280RhPrevious, config.bme280RhDiffReportingThreshold, COMOBJ_rhValue);
      writeIfDiff(bme280Press, &bme280PressPrevious, config.bme280PressDiffReportingThreshold, COMOBJ_pressValue);
      ...
      checkMinLimit(bme280Press, config.bme280PressMinLimit, config.bme280PressMinTriggeredValue, COMOBJ_pressMinLimitReached);
      checkMaxLimit(bme280Press, config.bme280PressMaxLimit, config.bme280PressMaxTriggeredValue, COMOBJ_pressMaxLimitReached);
  
      // Update CCS811 if temperature significantly differs from last uploaded
      {
        static float ccs811SetTemp = 0;
        static float ccs811SetRh = 0;
  ...
  //
  float max44009Lux;
  static float max44009LuxPrevious = 0;

  if(readMax44009)
  {
    max44009Lux = max44009.getLux();

    if(max44009Lux >= MAX44009_LUX_MIN && max44009Lux <= MAX44009_LUX_MAX)
    {
      const float reportingThreshold = max44009LuxPrevious * config.max44009LuxDiffReportingThresholdPct;
      writeIfDiff(max44009Lux, &max44009LuxPrevious, reportingThreshold, COMOBJ_luxValue);
  
      checkMaxLimit<uint32_t>(max44009Lux, config.max44009LuxMaxLimit, config.max44009LuxTriggeredValue, COMOBJ_luxMaxLimitReached);
  
      max44009.setHighThreshold(max(max44009Lux * (1.0 + config.max44009LuxDiffReportingThresholdPct), MAX44009_LUX_MAX));
      max44009.setLowThreshold(min(max44009Lux * (1.0 - config.max44009LuxDiffReportingThresholdPct), MAX44009_LUX_MIN));
    }
    else
      blinkLed(ERROR_MAX44009);
  ...
  // End of work loop, go back to sleep
